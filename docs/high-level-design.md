...
- **Current pay** comes from items whose date range covers today. A pay item applies from `effective_from` up to, but not including, `effective_to`, and an employee has at most one open item per component. The `pay_totals_on(date)` function gives each employee's annual total and annual gross pay (earnings, allowances and bonuses) in local currency on a date, and the `current_pay_totals` view applies it to today. The monthly equivalent is the annual total / 12, worked out by the shared helper with one rounding rule (D42). The directory and the dashboard read from it. If it ever becomes slow, the totals can be stored on the employee row and updated in the same transaction as each pay change.
...
| Brute force and load | Rate limits on login and forgot password, per IP address and per email, and on import and export, per user. |
...
| Headers | Helmet on the API. The web app's headers are set in `vercel.json`: a content security policy (scripts, requests and frames from the app only), frame, content type, referrer, permissions and opener policies. |
...
| Personal data | No government IDs, bank details, date of birth or gender. Logs never include pay amounts, passwords or the values of failed database queries. The demo uses synthetic data only. |
